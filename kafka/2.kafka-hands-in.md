...
UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa

# prompt 와 붙어 있으니 잘 확인할것

# user/pass         <-- Password 를 반드시 기억하세요.
## my-edu-admin / UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa
...
metadata:
  ...
  creationTimestamp: "2023-06-10T13:41:46Z"
  generation: 1
  labels:
    strimzi.io/cluster: my-cluster
  name: my-topic
  ...
  uid: 32cfa91d-3c34-4cad-b248-8b3b44c3c29c
spec:
  config:
...
status:
  conditions:
  - lastTransitionTime: "2023-06-10T13:41:46.705325883Z"
    status: "True"
    type: Ready
...
export PASSWORD=UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa        ## 개인별 passwrod 붙여넣자.   위 3.2 KafkaUser 를 참고하자. 
...
sasl_plain_password='UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa'             ## 개인별 passwrod 붙여넣자.   위 3.2 KafkaUser 를 참고하자. 
...
- node port 를 인식할 수 있는 본인 Bastion Server IP를 확인하자.
...
# nip 
my-cluster.kafka.localhost.35.247.230.92.nip.io
    ...
    listeners:
      ...
      - name: external
        ...
        configuration:
          ...
          brokers:
          - broker: 0
            advertisedHost: my-cluster.kafka.127.0.0.1.nip.io    # 각자의 Node IP로 변경
            nodePort: 32200
          - broker: 1
            advertisedHost: my-cluster.kafka.127.0.0.1.nip.io    # 각자의 Node IP로 변경
            nodePort: 32201
...
# 127.0.0.1  --> 본인ServerIP(35.247.230.92) 로 변경

# 변경
$ sed -i 's/127.0.0.1/35.247.230.92/' ./kafka/strimzi/kafka/13.kafka-ephemeral-auth-nodeport.yaml
...
...
          - broker: 0
            advertisedHost: my-cluster.kafka.35.247.230.92.nip.io    # 각자의 Node IP로 변경
            nodePort: 32200
          - broker: 1
            advertisedHost: my-cluster.kafka.35.247.230.92.nip.io    # 각자의 Node IP로 변경
            nodePort: 32201
...
...
  - addresses:
    - host: my-cluster.kafka.35.247.230.92.nip.io
      port: 32100
    bootstrapServers: my-cluster.kafka.35.247.230.92.nip.io:32100
    name: external
    type: external
...
```sh
bootstrap  : my-cluster.kafka.35.247.230.92.nip.io:32100
broker0    : my-cluster.kafka.35.247.230.92.nip.io:32200
broker1    : my-cluster.kafka.35.247.230.92.nip.io:32201
broker2    : my-cluster.kafka.35.247.230.92.nip.io:32202


# port 가 살아 있는지 netcat 으로 확인해 보자.
$ nc -zv my-cluster.kafka.35.247.230.92.nip.io 32100
Connection to my-cluster.kafka.35.247.230.92.nip.io (35.247.230.92) 32100 port [tcp/*] succeeded!
...
$ nc -zv my-cluster.kafka.35.247.230.92.nip.io 32203
...
kafkacat 을 podman 으로 실행한다.

```sh
# 실행
$ podman run --name kafkacat -d --user root docker.io/confluentinc/cp-kafkacat:latest sleep 365d
...
export BROKERS=my-cluster.kafka.35.247.230.92.nip.io:32100
export KAFKAUSER=my-edu-admin
export PASSWORD=UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa        ## 개인별 passwrod 붙여넣자.   위 3.2 KafkaUser 를 참고하자. 
export TOPIC=my-topic
...
Metadata for all topics (from broker -1: sasl_plaintext://my-cluster.kafka.35.247.230.92.nip.io:32100/bootstrap):
 3 brokers:
  broker 0 at my-cluster.kafka.35.247.230.92.nip.io:32200
  broker 2 at my-cluster.kafka.35.247.230.92.nip.io:32202
  broker 1 at my-cluster.kafka.35.247.230.92.nip.io:32201 (controller)
 1 topics:
  topic "my-topic" with 3 partitions:
    partition 0, leader 1, replicas: 1,2,0, isrs: 1,2,0
    partition 1, leader 0, replicas: 0,1,2, isrs: 1,2,0
    partition 2, leader 2, replicas: 2,0,1, isrs: 1,2,0
...
```sh
$ python
Python 3.9.17 (main, Jun  6 2023, 23:29:09)
[GCC 10.2.1 20210110] on linux
Type "help", "copyright", "credits" or "license" for more information.
>>>
...
# 개인환경으로 변경
bootstrap_servers='my-cluster.kafka.35.247.230.92.nip.io:32100'
sasl_plain_password='UIqVGInn7iZPo36zxJkZbRD3C4tyHXSa'             ## 개인별 passwrod 붙여넣자.   위 3.2 KafkaUser 를 참고하자. 
sasl_plain_username='my-edu-admin'
group_id='my-topic-group'
...
http://kafdrop.kafka.35.209.207.26.nip.io
...
http://grafana.kafka.35.209.207.26.nip.io
...
export PASSWORD=iUfOLiK9LM4QxwTMYnjOQHrG0gJiwQpa
export TOPIC=edu-topic02      # <-- 본인 타픽명 으로 지정
export GROUP=edu-topic-group
...
sasl_plain_password='iUfOLiK9LM4QxwTMYnjOQHrG0gJiwQpa'             ## 개인별 passwrod 붙여넣자.   위 3.2 KafkaUser 를 참고하자. 
topic_name='edu-topic01'      # <-- 본인 타픽명으로 지정
group_id='edu-group01'        # <-- 본인 그룹명으로 지정
...
bootstrap_servers='my-cluster.kafka.35.209.207.26.nip.io:32100'
sasl_plain_username='edu-user'
sasl_plain_password='iUfOLiK9LM4QxwTMYnjOQHrG0gJiwQpa'
topic_name='edu-topic01'      # <-- 본인 타픽명으로 지정
...
Spring에서 Kafka를 사용하는 방법에는 Apache Kafka 을 이용하거나 Apache Kafka Streams(Spring Cloud Stream) 을 이용하는 방법 등 여러가지가 있다.

본 실습에서는 kafka template을 이용할 것이다.
...
```sh
$ curl -X POST http://localhost:8081/publish?message=test


## 초당 1회 call 발생(producer )
$ while true; do curl -X POST http://localhost:8081/publish?message=test; date; echo;sleep 1; echo; done
...
* Partitioning 이라는 용어는 Record 가 어떤 Partition에게 전송될 것인지 선택하는 전략
* partitioner.class 종류
  * DefaultPartitioner
    * key 가 존재하면 hash 값을 기반으로 선택, key 가 존재하지 않으면 UniformStickyPartitioner 전략으로 구현되어 있음
  * RoundRobinPartitioner
    * Partition의 순서대로 하나씩 할당한다.
  * UniformStickyPartitioner
    * `batch.size` 가 가득 차거나 `linger.ms`가 경과할 때까지 특정 파티션에 sticky 하게 고정된다.
    * 데이터를 특정 Partition 으로 보내는 기능으로 Producer의 성능을 높일 수 있다.
    * kafka 2.4 버젼부터 지원되는 기능이다.
...
- python  로 Consumer 실행한다.
  ...
  - 이전에 실습했던 [python 실습 자료](#Bastion-Server-Container-run-★)를 참조한다.
